...
# the gait is a half circle
def semicircle_generator(radius, steps, reverse=False):
    assert (steps % 4) == 0  # spilt the half circle into many steps to finish
    ...
    # 前半部分，后移（只有y轴改变——腿接触地面）only the y changes
    for i in range(half_steps):
        result.append((0, radius - i*step_stride, 0))  # 直线 [radius, 0]

    # 第二（后半）部分，以半圆形状前移（y，z 改变）change y and z
    for i in range(half_steps):
